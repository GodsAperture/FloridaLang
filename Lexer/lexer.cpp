...
Token Lexer::next(){
    ...
    if(currChar == '('){
        ...
        if(currChar == '*'){
            ...
            file.get(currChar);
            column++;
            //While in a comment, check for *, if star is found, check to see if the next
            //character is ). If it is, then return. If not, continue.
            while(!file.eof()){
                file.get(currChar);
                column++;
                if(currChar == '*'){
    ...
    if(NumberQ(currChar)){
        ...
        while(true){
            ...
            //If the number ends with f, d, i, r, or l it will change the type of number.
            switch(currChar){
                case 'f':
                    inToken.append(currChar);
    ...
    //Identifers: Functions, variables, methods, patterns, and character based operators.
    if(AlphaQ(currChar)){
        ...
        while(true){
            ...
            //If the end character is an underscore, it will be appended and the type will be changed to Pattern and return.
            if(currChar == '_'){
                inToken.append(currChar);
            ...
            //If the end character is a letter or a number it is appended, else it will unget the character and return.
    ...
    if(SOperatorQ(currChar)){
        ...
        //and assume it could be a math related operator until proven otherwise.
        ...
        //If the operator is an arbitrary type operator, change its type to AOperator
        //and continually append shared and arbitrary operators.
        if(AOperatorQ(currChar)){
            ...
            //There are no more arbitrary operator or shared operator characters
